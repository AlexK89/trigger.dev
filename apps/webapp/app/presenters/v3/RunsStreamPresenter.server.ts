...
export class RunsStreamPresenter {
  ...
  public async call({
    ...
    logger.info("RunsStreamPresenter.call", {
      projectSlug,
    });
    ...
    return eventStream(request.signal, (send, close) => {
      ...
      subscriber.on("RUN_STATUS_CHANGED", async (message) => {
        safeSend({
          data: JSON.stringify({ source: message.source, ts: new Date().toISOString() }),
        });
      });
      ...
      return async function clear() {
        logger.info("RunsStreamPresenter.abort", {
          projectSlug,
        });
